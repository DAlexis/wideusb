...
TEST(NetworkLayerBinaryClass, Operating)
{
    ...
    net_layer.encode(sg1, 0xAAAAAAAA, 0xBBBBBBBB, 4);

    net_layer.encode(sg2, 0xCCCCCCCC, 0xDDDDDDDD, 3);

    sg1.push_back(sg2);
    std::vector<DecodedPacket> packets = net_layer.decode(BufferAccessor(sg1.merge()));
    ASSERT_EQ(packets.size(), 2);
    ASSERT_EQ(packets[0].sender, 0xAAAAAAAA);
    ASSERT_EQ(packets[0].receiver, 0xBBBBBBBB);
    ASSERT_EQ(packets[0].ttl, 4);
    ...
    ASSERT_EQ(packets[1].sender, 0xCCCCCCCC);
    ASSERT_EQ(packets[1].receiver, 0xDDDDDDDD);
    ASSERT_EQ(packets[1].ttl, 3);
