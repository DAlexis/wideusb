...
static USBD_CDC_RingBuffer ring_buffer;

static void clear_ring_buffer(USBD_CDC_RingBuffer* ring_buffer)
{
    memset(ring_buffer, 0, sizeof(USBD_CDC_RingBuffer));
}
...
TEST(RingBufferParsing, JSONDetection2)
{
    ...
    json_string = extract_json(&ring_buffer);
    ASSERT_TRUE(json_string.has_value());
    //cout << ">" << *json_string << "<" << endl;
