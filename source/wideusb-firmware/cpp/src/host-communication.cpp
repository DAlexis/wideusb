...
void HostCommunicator::input_parsing_thread_func()
{
    ...
    for (;;) {
        ...
        if (USBD_input_handler.buffer_on_timeout)
            continue;

        json = extract_possible_json(&USBD_input_handler.buffer);
...
{
    volatile uint32_t last_p_write = USBD_input_handler.buffer.p_write;
    if (os::get_os_time() - USBD_input_handler.last_receive_time > incoming_timeout)
    {
        ...
        // We may clear buffer.
        USBD_input_handler.buffer.p_read = last_p_write;
        return true;
    }
    return false;
}
